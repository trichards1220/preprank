...
Revision ID: a5a94be6e8d8
Revises: 
Create Date: 2026-03-29 08:08:45.402657
...
def upgrade() -> None:
    ...
    op.execute("""
        CREATE TABLE IF NOT EXISTS projected_ratings (
            ...
            projected_rating_p10 DECIMAL(6,2),
            projected_rating_p90 DECIMAL(6,2),
    ...
    op.execute("""
        CREATE TABLE IF NOT EXISTS game_impact_analysis (
            ...
            game_id INTEGER REFERENCES games(id),
            simulation_id INTEGER REFERENCES simulations(id) ON DELETE CASCADE,
            affected_team_id INTEGER REFERENCES teams(id),
    ...
    # Indexes
    op.execute("CREATE INDEX idx_games_season ON games(season_year, sport_id);")
    ...
    op.execute("CREATE INDEX idx_games_status ON games(status);")
    op.execute("CREATE INDEX idx_power_ratings_team ON power_ratings(team_id, season_year);")
    op.execute("CREATE INDEX idx_power_ratings_week ON power_ratings(week_number, season_year);")
    op.execute("CREATE INDEX idx_teams_school_sport ON teams(school_id, sport_id, season_year);")
    op.execute("CREATE INDEX idx_user_favorites_user ON user_favorites(user_id);")
    op.execute("CREATE INDEX idx_notifications_user ON notifications(user_id, read_status);")
    op.execute("CREATE INDEX idx_projected_ratings_sim ON projected_ratings(simulation_id);")
    op.execute("CREATE INDEX idx_projected_ratings_team ON projected_ratings(team_id);")
    op.execute("CREATE INDEX idx_game_predictions_game ON game_predictions(game_id);")
    op.execute("CREATE INDEX idx_game_impact_game ON game_impact_analysis(game_id, affected_team_id);")
...
def downgrade() -> None:
    """Downgrade schema."""
    op.execute("DROP TABLE IF EXISTS game_impact_analysis;")
    op.execute("DROP TABLE IF EXISTS game_predictions;")
    op.execute("DROP TABLE IF EXISTS projected_ratings;")
    op.execute("DROP TABLE IF EXISTS simulations;")
    op.execute("DROP TABLE IF EXISTS notifications;")
    op.execute("DROP TABLE IF EXISTS user_favorites;")
    op.execute("DROP TABLE IF EXISTS users;")
    op.execute("DROP TABLE IF EXISTS athlete_stats;")
    op.execute("DROP TABLE IF EXISTS athletes;")
    op.execute("DROP TABLE IF EXISTS power_ratings;")
